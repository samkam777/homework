...
int         traces_linked_list(log_t *plog_t, int tnot);
...
void        most_frequent_trace_frequency(log_t *plog_t, int tnot, int *mftf_idx, int *mftf);
void        print_the_most_frequent_trace(log_t *plog_t, int mftf_idx);
...
bool stage1(total_events **t_events, log_t *plog_t, int tnot)
{
    ...
    int removeSup = searchSupFromMatrix(*t_events, matrix, matrix_size, b.u, b.v);
...
stage0(log_t *plog_t, int tnot){
    ...
    int nodt = traces_linked_list(plog_t, tnot);
    /* Caculate the trace frequency */
    calculate_the_trace_frequency(plog_t, tnot);
    /* Most frequent trace frequency */
    int mftf_idx=0, mftf=0;
    most_frequent_trace_frequency(plog_t, tnot, &mftf_idx, &mftf);
    ...
    printf("Most frequent trace frequency: %d", mftf);
    print_blank();

    /* print the most frequent trace */
    print_the_most_frequent_trace(plog_t, mftf_idx);
    /* print number of all events */
    print_all_events(t_events);
    // print_stage_line(STAGE1);
...
int
traces_linked_list(log_t *plog_t, int tnot){
    /*
...
void
most_frequent_trace_frequency(log_t *plog_t, int tnot, int *mftf_idx, int *mftf){
    /*
     ...
     */
    for(int i=0; i<tnot; ++i){
        if(plog_t[i].trcs->freq > *mftf){
            *mftf = plog_t[i].trcs->freq;
            *mftf_idx = i;
        }
    }
}
...
void
print_the_most_frequent_trace(log_t *plog_t, int mftf_idx){
    /*
     * @brief find the most frequent trace
     * @param plog_t: original data log
     * @param mftf_idx: most frequent trace frequency index
     */
    event_t *mftf_head = plog_t[mftf_idx].trcs->head;
